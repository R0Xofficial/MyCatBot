...
async def github(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    github_link = "https://github.com/R0Xofficial/MyCatbot"; await update.message.reply_text(f"Meeeow! I'm open source! 💻 Here my code:\n{github_link}", disable_web_page_preview=True)
async def owner_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if OWNER_ID:
        ...
        message = (f"My designated human, the bringer of treats 🎁 and head scratches ❤️, is: 👤 <b>{owner_name}</b> ({owner_mention})\nThey hold the secret to the treat jar! ✨")
